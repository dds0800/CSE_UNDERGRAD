...
link removeNext(list my_list, link x) {
    ...
    if (x == NULL) // try to delete the first node
    {
        ...
        temp = x->next;
        x->next = x->next->next;  // IS THIS CODE SAFE? JUSTIFY YOUR ANSWER. This code is not safe, x->next may already be NULL.
        my_list->length -= 1;
    }
    return temp;
}
...
{
	link inserted_link = newLink(P,NULL);
	if(pos>=0)
    {
        if(pos==0)
            insertAtBeginning(A,inserted_link);
        ...
        else
        {
            ...
            for(i=getFirst(A);i!=NULL;i=getLinkNext(i))
            {
                if(j==(pos-1))
                {
                    previous=i;
            ...
            insertLink(A,previous,inserted_link);
        }
    }

    else
    {
        printf("\n\n***Invalid position. Nothing was done.***\n\n");
        return;
    }
}
...
void deleteOccurrences(list A, Item V)
{
	...
	// This loops iterates through list A.
	for(i=getFirst(A);i!=NULL;i=getLinkNext(i))
    {
        if(getLinkItem(i)==V)
        {
            if(i==getFirst(A))
            {
                link bad_node=removeFirst(A);
                destroyLink(bad_node);
                i=getFirst(A); // Start loop again

                /* This loop will check to see if the new next
                node is not an occurrence.*/
                while(getLinkItem(i)==V)
                {
                    bad_node=removeFirst(A);
                    destroyLink(bad_node);
                    i=getFirst(A);
                }
            }
            ...
            {
                link next=getLinkNext(i);
                link previous;
                link j;
                for(j=getFirst(A);j!=NULL;j=getLinkNext(j))
                {
                    if(getLinkItem(getLinkNext(j))==V)
                    {
                        previous=j;
                        break;
                    }
                }
                setLinkNext(previous,next);
                destroyLink(i);
                A->length-=1;
                i=getFirst(A); //Start Loop again
            }
...
list sublist(list A, list pos_list)
{
    ...
    // This loop iterates through the position list
    for(j=getFirst(pos_list);j!=NULL;j=(getLinkNext(j)))
    {
        int position = getLinkItem(j);
        k=0;

        //This loop iterates through List A
        for(i=getFirst(A);i!=NULL;i=getLinkNext(i))
        {
            link last_link;
            if(k==position)
            {
                if(getFirst(final_list)==NULL)
                {
                    link a_position = newLink(getLinkItem(i));
                    insertLink(final_list,NULL,a_position);
                    last_link = a_position;
                }

                else
                {
                    link a_position = newLink(getLinkItem(i));
                    insertLink(final_list,last_link,a_position);
                    last_link = a_position;
                }
            }
...
{
	link i;
	int list_max=0,max_count=0,j;

	//This loop will find the maximum in the list.
	for(i=getFirst(A);i!=NULL;i=getLinkNext(i))
    {
        if(getLinkItem(i)>list_max)
            list_max=getLinkItem(i);
    }

    //This loop will count the how many times the maximum occurs in the list.
    for(i=getFirst(A);i!=NULL;i=getLinkNext(i))
    {
        if(getLinkItem(i)==list_max)
            max_count++;
    ...
    deleteOccurrences(A,list_max);

    for(j=0;j<max_count;j++)
    {
        insertAtPosition(A,list_max,computeLength(A));
    }
}
